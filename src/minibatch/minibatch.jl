...
function update!(cache::BatchMHCache{A, B, C}, proposed_change::TrajectoryProposedChange, states, accept) where {A<:TransitionPathSampling.MetropolisHastings.GaussianMHTrajectoryCache, B, C}
    if accept
...
function TransitionPathSampling.MetropolisHastings.acceptance!(cache::BatchMHCache, states, alg::MinibatchMHAlg)
    ...
    info = Acceptance.minibatch_acceptance!(acceptance_cache, proposed_change, cache.observable, alg.bias);
    a = info.has_accepted
    cache.last_acceptance_info = info

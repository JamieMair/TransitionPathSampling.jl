...
@testset "Test cache generation" begin
    ...
    @testset "Check algorithm basics" begin
        ...
        @test typeof(cache)<:TPS.MetropolisHastings.GaussianMHTrajectoryCache
    ...
    @testset "Check cache parameter mask being set correctly" begin
        frac = 0.5
        alg = TPS.MetropolisHastings.gaussian_trajectory_algorithm(s, σ; params_changed_frac=frac);
        cache = TPS.generate_cache(alg, problem)
...
@testset "Test cache masking" begin
    ...
    function test_alg_masking(alg)
        ...
        TPS.MetropolisHastings.mask_cache!(cache, states)
        for i in changed_indices
    ...
    @testset "Test masking" begin
        alg = TPS.MetropolisHastings.gaussian_trajectory_algorithm(s, σ; params_changed_frac=0.5);
        test_alg_masking(alg)
    end
end
...
@testset "Test solve with trajectory algorithms" begin
    ...
    @testset "Test bridging + shooting algorithm" begin
        alg = TPS.MetropolisHastings.gaussian_trajectory_algorithm(s, σ; params_changed_frac=1.0, chance_to_shoot=0.5);
        problem = test_problem(n, d)
        @test typeof(solve(problem, alg, iter)) <: TPSSolution
    end
